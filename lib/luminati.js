...
function Luminati(opt, mgr){
    ...
    opt.whitelist_ips = _.map(opt.whitelist_ips, ip => new Netmask(ip));
...
function*_handler(_this, req, res, head){
    ...
    try {
        ...
        let ip = _this.get_req_remote_ip(req);
        if (!_.some(wips, wb => wb.contains(req.ip))) {
...
E.prototype.resolve_proxy = etask._fn(function*resolve_proxy(_this){
        ...
        Date.now()-timestamp<30000)
    {
        ...
        try {
            ...
            ips = yield etask.nfn_apply(dns, '.resolve', [domain]);
...
E.prototype._request = etask._fn(function*_request(_this, req, res, head){
    ...
    try {
        ...
        if (get_process && !req.process)
            _this.perr('no_process');
