...
E.prototype.handle_usage_start = function(data){
    if (!this.wss||!this._defaults.logs.value)
        return;
    const req = {
        uuid: data.uuid,
        details: {
...
E.prototype.logs_process = function(data){
    ...
    if (this.logs_queue.length<100)
        return;
    this.history_aggregator();
};

E.prototype.history_aggregator = function(){
    const _this = this;
    this.db.main.serialize(function(){
        _this.db.main.run('begin transaction');
        for (let row of _this.logs_queue)
            _this.db.main.stmt.history.run(row);
        _this.logs_queue = [];
        _this.db.main.run('commit');
        _this.db.main.run('vacuum');
    });
    this.logs_queue.ts = Date.now();
};
...
E.prototype.init = function(argv, run_config){
    ...
    const cmd_args = process.argv.slice(2).reduce((acc, el)=>{
        if (acc.last)
            return {res: acc.res.concat([[acc.last, el]])};
        return {res: acc.res, last: el};
    }, {res: []}).res;
    cmd_args.forEach(a=>ua.event('manager', 'argv_new', a[0]));
    ua.send();
};
...
E.prototype.count_logs_sql = etask._fn(function*count_logs(_this){
    const res = yield _this.sql(`SELECT COUNT(*) AS count FROM request`);
    return res[0].count;
});

E.prototype.get_size_sql = etask._fn(function*get_size_sql_mgr(_this){
    const p_count = yield _this.sql(`pragma page_count`);
    const p_size = yield _this.sql(`pragma page_size`);
    return Math.ceil(p_count[0].page_count*p_size[0].page_size/1024/1024);
});
...
E.prototype.save_config = function(){
    ...
    const proxies = this.proxies
        ...
        .map(p=>_.omit(p, qw`stats proxy_type zones _update
            www_whitelist_ips request_stats logs conflict`))
        .map(p=>_.omitBy(p, v=>!v && v!==0 && v!==false))
        .map(p=>_.omitBy(p, (v, k)=>{
            // empty proxy.whitelist_ips used to override
            // _defaults.whitelist_ips
            if (Array.isArray(v)&&!v.length&&k!='whitelist_ips')
                return true;
...
E.prototype.start_syncing_logs = function(){
    if (this._defaults.logs.value)
        this.syncing_logs_task = this.sync_logs();
};

E.prototype.sync_logs = etask._fn(function*mgr_sync_logs(_this){
    this.on('uncaught', e=>_this._log.error('sync_logs %s', zerr.e2s(e)));
    yield _this.adjust_logs_size();
    const now = Date.now();
    if (_this.logs_queue && now-_this.logs_queue.ts>=1000)
    {
        _this.history_aggregator();
        yield etask.sleep(1000);
    }
    else
        yield etask.sleep(now-_this.logs_queue.ts);
    if (_this.syncing_logs_task)
        _this.syncing_logs_task.return();
    _this.start_syncing_logs();
});
...
E.prototype.prepare_database = etask._fn(function*mgr_prepare_database(_this){
    ...
    try {
        const existing_hash = (yield _this.sql(`SELECT hash FROM schema_info
            LIMIT 1`))[0].hash;
...
E.prototype.prepare_db_stmt = function(){
    this.db.main.stmt = {
        ...
        history: this.db.main.prepare(`INSERT INTO request (uuid, port, url,
            ...
            content_type, remote_address, password)
...
function*mgr_get_password(_this, zone_name){
    try {
...
function*mgr_get_consts(_this, req, res){
    ...
    const pool_types = Object.keys(sessions.pool_types).map(pool_type=>{
    ...
    const log_levels = Object.keys(log.log_level).map(log_level=>{
...
function*mgr_send_rule_mail(_this, port, to, trigger, action, _url){
    if (!to)
        return;
    const jar = _this.luminati_jar.jar;
    yield etask.nfn_apply(request, [{url: _this.argv.api, jar}]);
...
E.prototype._complete_proxy_config = function(conf){
    let c = assign({}, E.default, this._defaults, conf,
        _.omit(this.argv.overlay_opt, 'password'));
    delete c.zones;
...
function*mgr_create_proxy(_this, proxy){
    ...
    proxy = assign({}, _.omit(_this._defaults, qw`zones`),
    ...
    conf.password = yield _this.get_password(conf.zone);
...
function*mgr_proxy_check(_this, new_proxy_config, old_proxy_port){
    ...
    const max_requests = check(new_proxy_config.max_requests);
    const session_duration = check(new_proxy_config.session_duration);
...
E.prototype.config_check = function(config){
    ...
    config.proxies.forEach(p=>{
        ...
        {
            const opt = {session: 'session_long', sequential: 'session_long'};
...
E.prototype.recent_ips = etask._fn(function*mgr_recent_ips(_this, req, res){
    // XXX krzysztof: decide if its needed and maybe remove
    const r = yield _this.json({
...
E.prototype.adjust_logs_size = etask._fn(
function*adjust_logs_mgr(_this, opt={}){
    const logs = _this._defaults.logs;
    if (logs.metric=='requests')
    {
        const limit = logs.value;
        const cur = yield _this.count_logs_sql();
        if (cur>=limit)
        {
            let x = 0;
            if (opt.delete_more)
                x = Math.ceil(limit/2);
            const to_delete = cur-limit+x;
            yield _this.delete_logs_sql(to_delete);
        }
    }
    else if (logs.metric=='megabytes')
    {
        const limit = logs.value;
        const cur = yield _this.get_size_sql();
        if (cur>=limit)
        {
            const reqs = yield _this.count_logs_sql();
            let f = 1;
            if (opt.delete_more)
                f = 2;
            const keep = Math.floor(limit*reqs/cur/f);
            const to_del = reqs-keep;
            yield _this.delete_logs_sql(to_del);
        }
    }
});
...
E.prototype.stats_clear_api = function(req, res){
...
E.prototype.add_whitelist_ip = function(req, res){
    if (req.ip!='127.0.0.1')
    ...
    this.save_config(this.get_settings());
    res.send('OK');
};
...
E.prototype.create_api_interface = function(){
    ...
    app.get('/config', this._api(this.config_get_api, ['root']));
    app.post('/config', this._api(this.config_set_api, ['root']));
    ...
    app.get('/recent_stats/reset', this._api(this.stats_clear_api));
    ...
    app.post('/add_whitelist_ip', this._api(this.add_whitelist_ip));
    return app;
};
...
E.prototype.set_location = etask._fn(function*mgr_set_location(_this){
    ...
    if (current_country && current_country.toLowerCase()!='cn')
        ...
        _this.opts.www_api = 'https://luminati-china.io';
});
...
class Ulimit_manager {
    constructor(mgr, opt){
        this.opt = assign({chk_fd_itv: 10*date.ms.SEC, log: 'warn'}, opt||{});
